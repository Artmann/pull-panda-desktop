import fs from 'fs'

import type { Octokit } from '@octokit/rest'

import { buildIssueCommentBody, buildSummaryBody, issueSlug } from './format'
import { findRobotThreads } from './threads'
import type { Issue, Review, ReviewThread, RobotThread } from './types'

const reviewThreadsQuery = `
  query($owner: String!, $repo: String!, $prNumber: Int!, $cursor: String) {
    repository(owner: $owner, name: $repo) {
      pullRequest(number: $prNumber) {
        reviewThreads(first: 100, after: $cursor) {
          pageInfo {
            hasNextPage
            endCursor
          }
          nodes {
            id
            isResolved
            comments(first: 1) {
              nodes {
                id
                databaseId
                body
                path
                line
              }
            }
          }
        }
      }
    }
  }
`

export function parseReviewContent(fileContent: string): Review {
  const raw = JSON.parse(fileContent)

  // opencode -f json wraps output in { "response": "..." }.
  const unwrapped = typeof raw.response === 'string'
  const content = unwrapped ? raw.response : JSON.stringify(raw)

  console.log(
    `Opencode JSON envelope: ${unwrapped ? 'unwrapped' : 'not present'}`
  )

  const jsonText = content
    .replace(/^```(?:json)?\s*\n?/, '')
    .replace(/\n?```\s*$/, '')

  return JSON.parse(jsonText)
}

function parseReviewFile(path: string): Review | null {
  console.log(`Parsing review file: ${path}`)

  if (!fs.existsSync(path)) {
    console.warn('review.json not found — skipping review comments.')

    return null
  }

  try {
    const review = parseReviewContent(fs.readFileSync(path, 'utf8'))

    console.log(`Parsed ${review.issues?.length ?? 0} issues from review file.`)

    return review
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
    console.warn(`Failed to parse review.json: ${message}`)

    return null
  }
}

async function fetchExistingReviewThreads(
  octokit: Octokit,
  owner: string,
  repo: string,
  prNumber: number
): Promise<ReviewThread[]> {
  const threads: ReviewThread[] = []
  let cursor: string | null = null

  while (true) {
    const result: Record<string, unknown> = await octokit.graphql(
      reviewThreadsQuery,
      { owner, repo, prNumber, cursor }
    )

    const reviewThreads = (
      result as {
        repository: {
          pullRequest: {
            reviewThreads: {
              nodes: ReviewThread[]
              pageInfo: { endCursor: string; hasNextPage: boolean }
            }
          }
        }
      }
    ).repository.pullRequest.reviewThreads

    threads.push(...reviewThreads.nodes)

    if (!reviewThreads.pageInfo.hasNextPage) {
      break
    }

    cursor = reviewThreads.pageInfo.endCursor
  }

  console.log(`Fetched ${threads.length} existing review threads.`)

  return threads
}

async function fetchPullRequestFilePaths(
  octokit: Octokit,
  owner: string,
  repo: string,
  prNumber: number
): Promise<Set<string>> {
  const { data: prFiles } = await octokit.rest.pulls.listFiles({
    owner,
    repo,
    pull_number: prNumber,
    per_page: 100
  })

  return new Set(prFiles.map((file) => file.filename))
}

// Filter issues to only include files that are part of the PR diff.
// The GitHub API returns 422 if a path doesn't match the diff.
export function filterIssuesInDiff(
  issues: Issue[],
  validPaths: Set<string>
): Issue[] {
  return issues.filter((issue) => {
    if (validPaths.has(issue.file)) {
      return true
    }

    console.warn(`Skipping issue on "${issue.file}" — not in PR diff.`)

    return false
  })
}

interface NewComment {
  body: string
  issue: Issue
  line: number
  path: string
}

// Handle issue comments — one thread per issue. Issues with an existing robot
// thread get their comment updated in place; the rest are returned as new
// comments to post. Matched threads are removed from `robotThreads` so the
// leftovers can be resolved as stale.
async function updateExistingThreads(
  octokit: Octokit,
  params: { owner: string; repo: string },
  issues: Issue[],
  robotThreads: Map<string, RobotThread>
): Promise<{ newComments: NewComment[]; updatedIssues: Issue[] }> {
  const newComments: NewComment[] = []
  const updatedIssues: Issue[] = []

  for (const issue of issues) {
    const slug = issueSlug(issue.file, issue.title)
    const body = buildIssueCommentBody(issue)
    const existing = robotThreads.get(slug)

    if (existing) {
      console.log(`Updating existing thread for: ${slug}`)

      await octokit.rest.pulls.updateReviewComment({
        owner: params.owner,
        repo: params.repo,
        comment_id: existing.commentId,
        body
      })

      robotThreads.delete(slug)
      updatedIssues.push(issue)
    } else {
      console.log(`New thread for: ${slug}`)

      newComments.push({
        body,
        issue,
        line: issue.line ?? 1,
        path: issue.file
      })
    }
  }

  return { newComments, updatedIssues }
}

// Resolve unresolved robot threads whose issues are no longer present.
async function resolveStaleThreads(
  octokit: Octokit,
  robotThreads: Map<string, RobotThread>
): Promise<void> {
  console.log(`Resolving ${robotThreads.size} stale threads.`)

  for (const [, thread] of robotThreads) {
    try {
      await octokit.graphql(
        `
        mutation($threadId: ID!) {
          resolveReviewThread(input: { threadId: $threadId }) {
            thread { id }
          }
        }
      `,
        { threadId: thread.threadId }
      )
    } catch (error) {
      console.warn(`Could not resolve thread ${thread.threadId}:`, error)
    }
  }
}

// Post new issue comments one at a time. The API rejects the entire batch
// if any line number falls outside the diff, so posting individually lets
// valid comments go through even when some lines can't be resolved.
async function postNewComments(
  octokit: Octokit,
  params: { commitSha: string; owner: string; prNumber: number; repo: string },
  newComments: NewComment[]
): Promise<Issue[]> {
  console.log(`Posting ${newComments.length} new comments as reviews.`)

  const postedIssues: Issue[] = []

  for (const comment of newComments) {
    try {
      await octokit.rest.pulls.createReview({
        owner: params.owner,
        repo: params.repo,
        pull_number: params.prNumber,
        commit_id: params.commitSha,
        event: 'COMMENT',
        comments: [
          { body: comment.body, line: comment.line, path: comment.path }
        ]
      })

      postedIssues.push(comment.issue)
    } catch (error) {
      console.warn(
        `Skipping comment on "${comment.path}:${comment.line}":`,
        error
      )
    }
  }

  return postedIssues
}

async function upsertSummaryComment(
  octokit: Octokit,
  params: { owner: string; prNumber: number; repo: string },
  postedIssueCount: number,
  summaryBody: string
): Promise<void> {
  const { owner, prNumber, repo } = params

  const { data: existingComments } = await octokit.rest.issues.listComments({
    owner,
    repo,
    issue_number: prNumber,
    per_page: 100
  })

  const existingSummary = existingComments.find((comment) =>
    comment.body?.includes('<!-- robot-code-review-summary -->')
  )

  console.log(
    `Summary comment: ${existingSummary ? 'updating existing' : 'creating new'} with ${postedIssueCount} issues.`
  )

  if (existingSummary) {
    await octokit.rest.issues.updateComment({
      owner,
      repo,
      comment_id: existingSummary.id,
      body: summaryBody
    })
  } else {
    await octokit.rest.issues.createComment({
      owner,
      repo,
      issue_number: prNumber,
      body: summaryBody
    })
  }
}

interface PostReviewParams {
  commitSha: string
  owner: string
  prNumber: number
  repo: string
  reviewPath: string
}

export async function postReview(
  octokit: Octokit,
  params: PostReviewParams
): Promise<void> {
  const { commitSha, owner, prNumber, repo, reviewPath } = params

  const review = parseReviewFile(reviewPath)

  if (!review) {
    return
  }

  const summary = review.summary ?? 'No summary provided.'
  const allIssues: Issue[] = review.issues ?? []

  console.log(
    `Review: ${allIssues.length} issues, summary length ${summary.length} chars.`
  )

  const validPaths = await fetchPullRequestFilePaths(
    octokit,
    owner,
    repo,
    prNumber
  )
  const issues = filterIssuesInDiff(allIssues, validPaths)

  console.log(`${issues.length} issues on files in the PR diff.`)

  const threads = await fetchExistingReviewThreads(
    octokit,
    owner,
    repo,
    prNumber
  )
  const robotThreads = findRobotThreads(threads)

  console.log(`Found ${robotThreads.size} existing robot threads.`)

  const { newComments, updatedIssues } = await updateExistingThreads(
    octokit,
    { owner, repo },
    issues,
    robotThreads
  )

  await resolveStaleThreads(octokit, robotThreads)

  const createdIssues = await postNewComments(
    octokit,
    { commitSha, owner, prNumber, repo },
    newComments
  )

  // Post or update the summary comment with only the issues that were posted.

  const postedIssues = [...updatedIssues, ...createdIssues]
  const summaryBody = buildSummaryBody(summary, postedIssues)

  await upsertSummaryComment(
    octokit,
    { owner, prNumber, repo },
    postedIssues.length,
    summaryBody
  )
}
