...
function buildReviewersGroup({
  ...
  for (const thread of reviewThreads) {
    const threadTask = buildAuthorScopedThreadTask(
      thread,
      comments,
      (login) => !isBotAuthor(login),
      threadSeverity
    )
...
function buildAuthorScopedThreadTask(
  thread: ReviewThread,
  comments: Comment[],
  matchesAuthor: (login: string | null | undefined) => boolean,
  severity: Severity
): ThreadTask | null {
